...
{RMeCab}はtaggerをラップしていて必要な情報を取れないので、[オレオレパッケージ](https://paithiov909.github.io/rjavacmecab/)を使います。動くはずです（ただし、{RMeCab}などに比べて5~10倍程度は遅い）。{RcppMeCab}でもできますが、公式のリポジトリのソースはWindows環境だとビルドにコケるのでUNIX系の環境が必要です（2020年5月現在）。
